...
def encontrar_materias_regulares(csv_path, csv_path_out):

    # dicionário para agrupar disciplinas por nome, semestre e curso
    disciplinas = {}
            ...
            if row[8] != "EAD" and validar_formato_tempo(row[9]):    
                ...
                if chave not in disciplinas:
                    ...
                    if disciplinas[chave]["turno"] != turno:
                        disciplinas[chave]["turno"] = None  # marcar como irregular
    ...
    with open(csv_path_out, 'w', newline='', encoding='utf-8') as file_out:
        ...
        for chave, dados in disciplinas.items():
            if dados["turno"] is not None:  # Apenas matérias regulares
                # Calcular a carga com base nos horários únicos
                carga = len(dados["horarios_unicos"])
                if carga > 4:
                    print(chave, carga)
                    print("\n")
                for row in dados["rows"]:
                    # Adicionar a carga ao final da linha
                    row.append(carga)
                    writer.writerow(row)
...
csv_path_out = Path(__file__).parent / 'resultados' / 'materias_regulares.csv'

# Executar a função
encontrar_materias_regulares(csv_path, csv_path_out)
